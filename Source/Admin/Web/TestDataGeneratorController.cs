...
namespace Web
{
    ...
    public class TestDataGeneratorController
    {
        ...
        public TestDataGeneratorController(IEventEmitter eventEmitter)
        {
        ...
        public void CreateUsers()
        {
            ...
            foreach (var id in _userIds)
                _eventEmitter.Emit("User", new UserCreated() { Id = id, Firstname = $"User {i + 1}", Lastname = "Doe", NationalSocietyId = _nationalSocietyIds[i++ % _nationalSocietyIds.Length], Country = "Norway" });
        }
    }
}
