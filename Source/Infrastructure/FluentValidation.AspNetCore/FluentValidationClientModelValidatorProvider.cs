...
{
    public delegate IClientModelValidator FluentValidationClientValidatorFactory(ClientValidatorProviderContext context,
    ...
    public class FluentValidationClientModelValidatorProvider : IClientModelValidatorProvider
    {
        ...
        private readonly Dictionary<Type, FluentValidationClientValidatorFactory> _validatorFactories =
            new Dictionary<Type, FluentValidationClientValidatorFactory>()
            {
                {typeof(INotNullValidator), (context, rule, validator) => new RequiredClientValidator(rule, validator)},
                {
                    typeof(INotEmptyValidator),
                    (context, rule, validator) => new RequiredClientValidator(rule, validator)
                },
                ...
                {typeof(IEmailValidator), (context, rule, validator) => new EmailClientValidator(rule, validator)},
                {
                    ...
                    (context, rule, validator) => new RegexClientValidator(rule, validator)
                },
                {
                    typeof(MaximumLengthValidator),
                    (context, rule, validator) => new MaxLengthClientValidator(rule, validator)
                },
                {
                    typeof(MinimumLengthValidator),
                    (context, rule, validator) => new MinLengthClientValidator(rule, validator)
                },
                {
                    typeof(LengthValidator),
                    (context, rule, validator) => new StringLengthClientValidator(rule, validator)
                },
                {
                    typeof(ExactLengthValidator),
                    (context, rule, validator) => new StringLengthClientValidator(rule, validator)
                },
                {
                    typeof(InclusiveBetweenValidator),
                    (context, rule, validator) => new RangeClientValidator(rule, validator)
                },
                {
                    typeof(GreaterThanOrEqualValidator),
                    (context, rule, validator) => new RangeMinClientValidator(rule, validator)
                },
                {
                    typeof(LessThanOrEqualValidator),
                    (context, rule, validator) => new RangeMaxClientValidator(rule, validator)
                },
        ...
        {
            var isNonNullableValueType = !TypeAllowsNullValue(context.ModelMetadata.ModelType);
            ...
            if (isNonNullableValueType)
            {
                ...
                if (fvHasRequiredRule)
                {
                    var dataAnnotationsRequiredRule = context.Results
                        .FirstOrDefault(x =>
                            x.Validator is Microsoft.AspNetCore.Mvc.DataAnnotations.Internal.RequiredAttributeAdapter);
                    context.Results.Remove(dataAnnotationsRequiredRule);
                }
            }
        }

        protected virtual IClientModelValidator GetModelValidator(ClientValidatorProviderContext context,
            PropertyRule rule, IPropertyValidator propertyValidator)
        {
            ...
            if (factory != null)
            {
                var ruleSetToGenerateClientSideRules =
                    RuleSetForClientSideMessagesAttribute.GetRuleSetsForClientValidation(_httpContextAccessor
                        ?.HttpContext);
                var executeDefaultRule =
                    ruleSetToGenerateClientSideRules.Contains("default", StringComparer.OrdinalIgnoreCase) &&
                    string.IsNullOrEmpty(rule.RuleSet);
                var shouldExecute = ruleSetToGenerateClientSideRules.Contains(rule.RuleSet) || executeDefaultRule;
