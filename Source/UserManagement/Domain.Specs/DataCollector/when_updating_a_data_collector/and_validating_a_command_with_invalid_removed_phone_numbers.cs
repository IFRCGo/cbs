...
namespace Domain.Specs.DataCollector.when_updating_a_data_collector
{
    ...
    public class and_validating_a_command_with_invalid_removed_phone_numbers
    {
        ...
        Establish context = () => 
        {
            ...
            cmd = given.a_command_builder.get_invalid_command(
                (cmd) => cmd.PhoneNumbersRemoved = new string[1] {string.Empty}
            );
        };

        Because of = () => { validation_results = validator.Validate(cmd); };
        It should_be_invalid = () => validation_results.ShouldBeInvalid();
        It should_identify_the_phone_numbers_as_the_problem = () => validation_results.ShouldHaveInvalidProperty(nameof(cmd.PhoneNumbersRemoved));
    }
}