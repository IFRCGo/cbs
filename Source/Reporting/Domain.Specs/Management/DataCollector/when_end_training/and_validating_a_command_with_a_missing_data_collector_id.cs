...
namespace Domain.Specs.Management.DataCollector.when_end_training
{
    [Subject("EndTraining")]
    public class and_validating_a_command_with_a_missing_data_collector_id
    {
        static EndTraining cmd;
        static EndTrainingInputValidator validator;
        static ValidationResult validation_results;

        Establish context = () =>
        {
            validator = new EndTrainingInputValidator();

            cmd = when_end_training.given.a_command_builder.get_invalid_command((cmd) => cmd.DataCollectorId = Guid.Empty);
        };
