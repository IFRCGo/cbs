...
namespace Events.Reporting.CaseReports
{
    public class CaseReportReceived : IEvent
    {
        ...
        public CaseReportReceived(Guid dataCollectorId, Guid healthRiskId, string healthRiskName,
            string origin, string message, int numberOfMalesUnder5, int numberOfMalesAged5AndOlder, 
            int numberOfFemalesUnder5, int numberOfFemalesAged5AndOlder, string region, string district,
            string village, double longitude, double latitude, DateTimeOffset timestamp)
        {
