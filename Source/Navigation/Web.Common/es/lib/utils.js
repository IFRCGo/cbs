 ...
 */
function bumpSort(arr, valueToBump) {
  var arrCopy = arr.slice();
  var valueIndex = arrCopy.indexOf(valueToBump);

  if (valueIndex === -1) {
    return arrCopy.sort();
  }

  var arrOfBumped = arrCopy.splice(valueIndex, 1);
...
function getPageTitle(subtitle) {
  ...
  if (subtitle) {
    pageTitle = "".concat(subtitle, " | ").concat(process.env.REACT_APP_SITE_TITLE);
  } else {
...
function parseQueryString(query) {
  return query.split('&').reduce(function (prev, pair) {
...
function formatDate(dateString) {
  return (0, _moment.default)(dateString).format('DD.MM.YYYY');
}

function formateTime(dateString) {
  return (0, _moment.default)(dateString).format('HH:mm');
}